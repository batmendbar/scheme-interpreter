...
void *talloc(size_t size){
  ...
  Value* storePointer = malloc(sizeof(Value));
  Value* newHead = malloc(sizeof(Value));
  void* memoryAlloc = malloc(size);

  storePointer->type = PTR_TYPE;
  storePointer->p = memoryAlloc;
  
  newHead->type = CONS_TYPE; 
  newHead->c.car = storePointer;
  newHead->c.cdr = head;
